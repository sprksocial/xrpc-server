...
Deno.test({
  ...
  async fn() {
    ...
    const badClient = new XrpcClient(`http://localhost:${port}`, MISMATCHED_LEXICONS)
    ...
    await Deno.test("serves requests", async () => {
      ...
      const fooError = await client.call('io.example.error', { which: 'foo' }).catch(e => e)
      ...
      const barError = await client.call('io.example.error', { which: 'bar' }).catch(e => e)
      ...
      const falsyError = await client.call('io.example.throwFalsyValue').catch(e => e)
      ...
      const otherError = await client.call('io.example.error', { which: 'other' }).catch(e => e)
      ...
      const invalidError = await client.call('io.example.invalidResponse').catch(e => e)
      ...
      assertEquals(invalidError.validationError.message, 'Output must have the property "expectedValue"')
      ...
      const upstreamError = await client.call('io.example.invalidUpstreamResponse').catch(e => e)
    ...
    await Deno.test("serves error for missing/mismatch schemas", async () => {
      ...
      const procError = await badClient.call('io.example.procedure').catch(e => e)
      ...
      const notFoundError = await badClient.call('io.example.doesNotExist').catch(e => e)
