...
import { assertEquals, assertRejects, assertGreater } from "jsr:@std/assert"
...
Deno.test({
  ...
  async fn() {
    ...
    server.streamMethod('io.example.streamOne', async function* ({ params }: { params: xrpcServer.Params }) {
    ...
    server.streamMethod('io.example.streamTwo', async function* ({ params }: { params: xrpcServer.Params }) {
      const countdown = Number(params.countdown ?? 0)
      for (let i = countdown; i >= 0; i--) {
        await wait(200)
        yield {
    ...
    try {
      ...
      await Deno.test("closing websocket server while client connected", async (t) => {
        ...
        await t.step("uses heartbeat to reconnect if connection dropped", async () => {
