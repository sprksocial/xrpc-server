  ...
  async fn() {
    ...
    await Deno.test("verifyJwt tests", async (t) => {
      await t.step("fails on expired jwt", async () => {
        ...
        await assertRejects(
          () => xrpcServer.verifyJwt(jwt, 'did:example:aud', null, async () => await keypair.did()),
          Error,
          'jwt expired'
        )
      })
      ...
      await t.step("fails on bad audience", async () => {
        ...
        await assertRejects(
          () => xrpcServer.verifyJwt(jwt, 'did:example:aud2', null, async () => await keypair.did()),
          Error,
      ...
      await t.step("fails on bad lxm", async () => {
        ...
        await assertRejects(
          () => xrpcServer.verifyJwt(jwt, 'did:example:aud1', 'com.atproto.repo.putRecord', async () => await keypair.did()),
          Error,
      ...
      await t.step("interoperates with jwts signed by other libraries", async () => {
